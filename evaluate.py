...
def uruchom_XD(nazwa_skryptu, sciezka_obrazka, prawdziwy):
    try:
        wynik = subprocess.check_output(
            ["python", nazwa_skryptu, sciezka_obrazka, prawdziwy],
            text=True,
...
def uruchom_skrypt(nazwa_skryptu, sciezka_obrazka):
    try:
        wynik = subprocess.check_output(
            ["python", nazwa_skryptu, sciezka_obrazka],
            text=True,
            stderr=subprocess.STDOUT,
        )
...
if __name__ == "__main__":
    ...
    for i, prawdziwa_tablica in enumerate(lista_tablic, start=1):
        ...
        wynik_szablony = uruchom_XD("XD.py", sciezka_pliku, prawdziwa_tablica)
        ...
        print(
            f"{i:02d}_image | {prawdziwa_tablica:<10} | {wynik_szablony:<20} | {wynik_tesseract}"
        )
    ...
    print(
        f"Skuteczność pojedynczych znaków (Accuracy): {(podobie_szablony / liczba_testow):.2f}%"
    )

    print("\n--- ŚCIEŻKA 2: MODEL REFERENCYJNY (TESSERACT OCR) ---")
    print(
        f"Tablice odczytane w 100%: {(poprawne_tesseract / liczba_testow) * 100:.2f}%"
    )
    print(
        f"Skuteczność pojedynczych znaków (Accuracy): {(podobie_tesseract / liczba_testow):.2f}%"
    )
    print("=" * 50)
